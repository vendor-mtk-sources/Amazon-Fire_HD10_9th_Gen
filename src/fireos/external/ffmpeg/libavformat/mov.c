...
static int mov_read_mhac(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    /*set the codec id*/
    st->codecpar->codec_id = AV_CODEC_ID_XHEAAC;

     /*first 5 bytes are descriptive and not needed for extra data*/
    avio_r8(pb);
    avio_rb32(pb);
    config_size -= 5; /*size of previous reads */

    /* remaining is decoder config data*/
    if (par->extradata) {
        av_log(c->fc, AV_LOG_ERROR, "extradata already found\n");
        return AVERROR_INVALIDDATA;
    }

    /*make space for config*/
    par->extradata = av_mallocz(config_size + AV_INPUT_BUFFER_PADDING_SIZE);
    if (!par->extradata)
        return AVERROR(ENOMEM);

    par->extradata_size = config_size;

    /*extract config from stream*/
    char buf[256] = {0};
    int err = ffio_read_size(pb, buf, config_size);

    if (err < 0) {
        par->extradata_size -= config_size;
        return err;
    } else if (err < config_size) {
        av_log(c->fc, AV_LOG_WARNING, "error reading extradata\n");
        par->extradata_size -= config_size - err;
    }

    /*populate extradata*/
    memcpy(par->extradata, buf, par->extradata_size);
