...
#define RAW_SAMPLES     1024 /* fosmod_amazon_wha */
...
smv_out:
    ...
    /* copied from libavformat/pcm.c */
    /*
     * Compute read size to complete a read every 40ms.
     * Clamp to RAW_SAMPLES if larger.
     */
    size = FFMAX(st->codecpar->sample_rate/25, 1);
    size = FFMIN(size, RAW_SAMPLES) * st->codecpar->block_align;
