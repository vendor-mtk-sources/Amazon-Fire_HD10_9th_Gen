 ...
 * Portions copyright (C) 2015-2017 Amazon.com, Inc. or its affiliates
 *
...
/* Amazon modification begins */
#if !defined(ANDROID) && !defined(__ANDROID__)
#ifndef G_OS_WIN32
#include <iconv.h>
#endif
#endif
/* Amazon modification ends */
#include <errno.h>
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
/* Amazon modification begins */
#else
#define FAKE_ICONV_IDENTITY     ((GIConv)1)
#define FAKE_ICONV_LATIN1_UTF8  ((GIConv)2)
#define FAKE_ICONV_UTF8_LATIN1  ((GIConv)3)
#endif
/* Amazon modification ends */
...
/* Amazon modification begins */
  ...
  if (strcmp(to_codeset, from_codeset) == 0)
    return FAKE_ICONV_IDENTITY;
  if (strcmp(to_codeset, "utf8") == 0 && strcmp(from_codeset, "latin1") == 0)
    return FAKE_ICONV_LATIN1_UTF8;
  if (strcmp(to_codeset, "latin1") == 0 && strcmp(from_codeset, "utf8") == 0)
    return FAKE_ICONV_UTF8_LATIN1;
  return (GIConv)-1;
#endif
/* Amazon modification ends */
}
...
/* Amazon modification begins */
  ...
  /* If inbuf is NULL, reset the converter, but nothing needs doing here */
  if (!inbuf || !*inbuf)
    return 0;

  if (!outbuf || !*outbuf || !inbytes_left || !outbytes_left)
  {
    errno = E2BIG;
    return -1;
  }
  ...
  maxbytes = (inbytes > outbytes) ? outbytes : inbytes;

  /* Just handle very basic conversion of the ASCII parts of Latin1 and UTF8 */
  switch ((ssize_t)converter)
  {
    case (ssize_t)FAKE_ICONV_IDENTITY:
      memcpy(outptr, inptr, maxbytes);
      copiedbytes = maxbytes;
      break;
    case (ssize_t)FAKE_ICONV_LATIN1_UTF8:
    case (ssize_t)FAKE_ICONV_UTF8_LATIN1:
      copiedbytes = 0;
      while (copiedbytes < maxbytes && (in = *inptr) >= 0)
      {
        inptr++;
        *(outptr++) = in;
        copiedbytes++;
      ...
      break;
    default:
      /* Do nothing, leaving copiedbytes as zero */
      ;
  }

  *inbuf += copiedbytes;
  *outbuf += copiedbytes;
  *inbytes_left -= copiedbytes;
  *outbytes_left -= copiedbytes;
