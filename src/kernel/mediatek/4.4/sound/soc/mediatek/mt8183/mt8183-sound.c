...
static void Aud_IRQ2_Handler(void)
{
	...
	if (GetMemoryPathEnable(Soc_Aud_Digital_Block_MEM_AWB2))
		Auddrv_AWB2_Interrupt_Handler();
...
int set_ap_dmic_vul2(bool enable)
{
		...
		if (enable) {
			if (GetMemoryPathEnable(Soc_Aud_Digital_Block_ADDA_UL2)) {
				...
				// Set sample rate(16K)
				Afe_Set_Reg(AFE_ADDA6_UL_SRC_CON0, 0x1 << 17, 0x7 << 17);

				// Turns on digital microphone for channel 1 and channel 2
				Afe_Set_Reg(AFE_ADDA6_UL_SRC_CON0, 0x3 << 21, 0x3 << 21);
				...
				// Set ch1/ch2 digital mic low power mode : 1.625M
				Afe_Set_Reg(AFE_ADDA6_UL_SRC_CON0, 0x1 << 14, 0x3 << 14);

				// Set digmic input mode : 1.625M
				Afe_Set_Reg(AFE_ADDA6_UL_SRC_CON0, 0x1 << 5, 0x1 << 5);
				...
				/* Add for DMIC FIFO input */
				// ul_fifo_digmic_wdata_testen
				//Afe_Set_Reg(AFE_ADDA_UL_DL_CON0, 0x1 << 4, 0x1 << 4);
				// ul_fifo_digmic_testin
				//Afe_Set_Reg(AFE_ADDA_UL_DL_CON0, 0x3 << 5, 0x3 << 5);
			}
		}
