...
static void set_lr_trim_code(void)
{
	...
	if (hpl_base > 0 || hpr_base > 0) {
		...
		if (code_change) {
			...
			mdelay(10);
			if (hpl_base > 0)
				trimcodel = (hpl_base == hpl_min) ? 0x0 : (((abs(hpl_base)*3)/abs(hpl_base-hpl_min))+1)/2;

			if (hpr_base > 0)
				trimcoder = (hpr_base == hpr_min) ? 0x0 : (((abs(hpr_base)*3)/abs(hpr_base-hpr_min))+1)/2;
		}
	}
	if (hpl_base < 0 || hpr_base < 0) {
		if (hpl_base < 0) {
		...
		if (code_change) {
			...
			mdelay(10);
			if (hpl_base < 0)
				trimcodel = (hpl_base == hpl_min) ? 0x0 : (((abs(hpl_base)*3)/abs(hpl_base-hpl_min))+1)/2 + 8;
			if (hpr_base < 0)
				trimcoder = (hpr_base == hpr_min) ? 0x0 : (((abs(hpr_base)*3)/abs(hpr_base-hpr_min))+1)/2 + 8;
		}
	}
...
static void set_lr_trim_code_spk(int channel)
{
	...
	if (hpl_base > 0) {
		...
		mdelay(10);
		trimcode = (hpl_base == hpl_min) ? 0x0 : (((abs(hpl_base)*3)/abs(hpl_base-hpl_min))+1)/2;
	} else {
		...
		hpl_min = get_spk_trim_offset(channel);
		mdelay(10);
		trimcode = (hpl_base == hpl_min) ? 0x0 : (((abs(hpl_base)*3)/abs(hpl_base-hpl_min))+1)/2 + 8;
...
static int Ext_Speaker_Amp_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	int retval;
	pr_aud("%s() switch = %ld\n ", __func__, ucontrol->value.integer.value[0]);
	if (strcmp(CONFIG_ARCH_MTK_PROJECT, "maverick") == 0) {
...
static int Ext_Headphone_Amp_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	if ((ucontrol->value.integer.value[0] == true) &&
		...
		Ext_Headphone_Amp_Change(ucontrol->value.integer.value[0]);
		mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_EXTHPAMP] =
		    ucontrol->value.integer.value[0];
	} else if ((ucontrol->value.integer.value[0] == false) &&
		(mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_EXTHPAMP] ==
