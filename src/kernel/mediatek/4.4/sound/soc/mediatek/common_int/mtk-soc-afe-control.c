...
static DEFINE_SPINLOCK(afe_mem_blk_awb2_lock);
...
extern int mt_aud_i2s2_bit_width(void);
...
/* i2s2 adc2 data */
bool SetI2s2Adc2Out(unsigned int SampleRate)
{
	struct audio_digital_i2s DigtalI2SIn;
	bool mI2S2WLen;
	pr_warn("%s(), jhrock1031", __func__);

	if (mt_aud_i2s2_bit_width()) {
		mI2S2WLen = Soc_Aud_I2S_WLEN_WLEN_32BITS;
	} else {
		mI2S2WLen = Soc_Aud_I2S_WLEN_WLEN_16BITS;
	}
	...
	DigtalI2SIn.mI2S_WLEN = mI2S2WLen;
...
#ifdef CONFIG_SND_PAM8908_HP_AMP
	if (!mt_soc_i2s1_clk_always_on() ) {
...
bool checkUplinkMEMIfStatus(void)
{
	return mAudioMEMIF[Soc_Aud_Digital_Block_MEM_VUL]->mState ||
		...
		mAudioMEMIF[Soc_Aud_Digital_Block_MEM_AWB2]->mState ||
...
void Auddrv_AWB2_Interrupt_Handler(void)
{
	struct afe_mem_control_t *Mem_Block = AFE_Mem_Control_context[Soc_Aud_Digital_Block_MEM_AWB2];
	kal_uint32 HW_Cur_ReadIdx = 0;
	kal_uint32 MaxCopySize = 0;
	kal_int32 Hw_Get_bytes = 0;
	struct substream_list *temp = NULL;
	...
	if (GetMemoryPathEnable(Soc_Aud_Digital_Block_MEM_AWB2) == false) {
		/* printk("%s(), GetMemoryPathEnable(Soc_Aud_Digital_Block_MEM_AWB2) == false, return\n ", __func__); */
		spin_unlock_irqrestore(&Mem_Block->substream_lock, flags);
		pr_err("-%s(), GetMemoryPathEnable(Soc_Aud_Digital_Block_MEM_AWB2) = %d\n ",
		       __func__, GetMemoryPathEnable(Soc_Aud_Digital_Block_MEM_AWB2));
		return;
	}

	mBlock = &Mem_Block->rBlock;
	HW_Cur_ReadIdx = word_size_align(Afe_Get_Reg(AFE_AWB2_CUR));
	PRINTK_AUD_AWB2("+%s HW_Cur_ReadIdx = 0x%x\n ", __func__, HW_Cur_ReadIdx);
	...
	MaxCopySize = Get_Mem_MaxCopySize(Soc_Aud_Digital_Block_MEM_AWB2);
	PRINTK_AUD_AWB2("1  mBlock = %p MaxCopySize = 0x%x u4BufferSize = 0x%x\n", mBlock,
		       MaxCopySize, mBlock->u4BufferSize);
	...
	if (MaxCopySize) {
		...
		mBlock->u4DMAReadIdx += MaxCopySize;
		mBlock->u4DMAReadIdx %= mBlock->u4BufferSize;
		Clear_Mem_CopySize(Soc_Aud_Digital_Block_MEM_AWB2);
		PRINTK_AUD_AWB2("%s read  ReadIdx:0x%x, WriteIdx:0x%x,BufAddr:0x%x  CopySize =0x%x\n",
			       __func__, mBlock->u4DMAReadIdx, mBlock->u4WriteIdx,
			       mBlock->pucPhysBufAddr, mBlock->u4MaxCopySize);
	}
	...
	spin_unlock_irqrestore(&Mem_Block->substream_lock, flags);
	PRINTK_AUD_AWB2("-%s u4DMAReadIdx:0x%x, u4WriteIdx:0x%x mBlock->u4DataRemained = 0x%x\n",
		       __func__, mBlock->u4DMAReadIdx, mBlock->u4WriteIdx, mBlock->u4DataRemained);
}
...
void mem_blk_spinlock(enum soc_aud_digital_block mem_blk)
{
	...
	case Soc_Aud_Digital_Block_MEM_AWB:
		...
		break;
	case Soc_Aud_Digital_Block_MEM_AWB2:
		spin_lock_irqsave(&afe_mem_blk_awb2_lock, spinlock_flags[Soc_Aud_Digital_Block_MEM_AWB2]);
		break;
	case Soc_Aud_Digital_Block_MEM_MOD_DAI:
...
void mem_blk_spinunlock(enum soc_aud_digital_block mem_blk)
{
	...
	case Soc_Aud_Digital_Block_MEM_AWB:
		...
		break;
	case Soc_Aud_Digital_Block_MEM_AWB2:
		spin_unlock_irqrestore(&afe_mem_blk_awb2_lock, spinlock_flags[Soc_Aud_Digital_Block_MEM_AWB2]);
		break;
	case Soc_Aud_Digital_Block_MEM_MOD_DAI:
