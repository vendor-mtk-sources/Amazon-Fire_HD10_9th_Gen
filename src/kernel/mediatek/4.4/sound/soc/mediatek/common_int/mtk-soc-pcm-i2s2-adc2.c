...
static void StopAudioI2S2adc2Hardware(struct snd_pcm_substream *substream)
{
	...
	if (substream->runtime->channels > 2) {
		SetIntfConnection(Soc_Aud_InterCon_DisConnect,
			Soc_Aud_AFE_IO_Block_I2S2, Soc_Aud_AFE_IO_Block_MEM_VUL);
		SetIntfConnection(Soc_Aud_InterCon_DisConnect,
			Soc_Aud_AFE_IO_Block_I2S2_2, Soc_Aud_AFE_IO_Block_MEM_VUL_DATA2);
	} else {
		SetIntfConnection(Soc_Aud_InterCon_DisConnect,
			Soc_Aud_AFE_IO_Block_I2S2_2, Soc_Aud_AFE_IO_Block_MEM_VUL_DATA2);
	}
...
static snd_pcm_uframes_t mtk_i2s2_adc2_pcm_pointer(struct snd_pcm_substream *substream)
{
	...
	HW_memory_index = (HW_Cur_ReadIdx - I2S2_ADC2_Control_context->rBlock.pucPhysBufAddr);

	return bytes_to_frames(substream->runtime, HW_memory_index);
}
