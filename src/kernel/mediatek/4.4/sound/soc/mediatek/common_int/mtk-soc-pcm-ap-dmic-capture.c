 ...
 * Description:
 * ------------
 *   Audio Ul1 data1 uplink for ap dmic
 *
 * Author:
 * -------
 * Chipeng Chang
...
/* information about */
struct afe_mem_control_t  *VUL_Control_context_ap_dmic;
...
int set_ap_dmic_vul2(bool enable);
...
static snd_pcm_uframes_t mtk_capture_pcm_pointer(struct snd_pcm_substream *substream)
{
	return get_mem_frame_index(substream,
		VUL_Control_context_ap_dmic, cap_mem_blk);
}
				     ...
				     struct snd_pcm_hw_params *hw_params)
{
	...
	set_mem_block(substream, hw_params, VUL_Control_context_ap_dmic, cap_mem_blk);
...
static int mtk_capture_pcm_open(struct snd_pcm_substream *substream)
{
	...
	AudDrv_Clk_On();
	VUL_Control_context_ap_dmic = Get_Mem_ControlT(cap_mem_blk);
...
static int mtk_capture_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node)
		dev_set_name(&pdev->dev, "%s", MT_SOC_AP_DMIC_PCM);
...
static int mtk_afe_capture_probe(struct snd_soc_platform *platform)
{
	cap_mem_blk = Soc_Aud_Digital_Block_MEM_AWB2;
	cap_mem_blk_io = Soc_Aud_AFE_IO_Block_MEM_AWB2;
...
static const struct of_device_id mt_soc_pcm_capture_of_ids[] = {
	{ .compatible = "mediatek,mt_soc_pcm_ap_dmic_capture", },
	{}
};
#endif
...
static int __init mtk_soc_capture_platform_init(void)
{
	...
	pr_warn("%s\n", __func__);
#ifndef CONFIG_OF
	soc_mtkafe_capture_dev = platform_device_alloc(MT_SOC_AP_DMIC_PCM, -1);
