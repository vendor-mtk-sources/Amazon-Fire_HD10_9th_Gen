...
// As-is from definition with TI PPC3 register dump
#define CFG_META_DELAY  (254)

#include "tas5805m.h"
#include "tas5805m_abc123.h"

#define TAS5805M_DRV_NAME    "tas5805m"

#define TAS5805M_RATES	     (SNDRV_PCM_RATE_48000 | SNDRV_PCM_RATE_96000)
...
void tas5805m_process_fault(struct snd_soc_codec *codec)
{
	...
	if (chan_fault_reg || pvdd_fault_reg || oc_fault_reg) {
		...
		pr_warn("%s: Processed Tweeter AMP fault\n", __func__);
	}
}
EXPORT_SYMBOL(tas5805m_process_fault);

static int tas5805m_get_die_revision(struct snd_soc_codec *codec)
{
	unsigned int die_rev;
	snd_soc_write(codec, TAS5805M_REG_00, TAS5805M_PAGE_00);
	snd_soc_write(codec, TAS5805M_REG_7F, TAS5805M_BOOK_00);
	snd_soc_write(codec, TAS5805M_REG_7D, TAS5805M_PAGE_11);
	snd_soc_write(codec, TAS5805M_REG_7E, TAS5805M_PAGE_FF);
	snd_soc_write(codec, TAS5805M_REG_00, TAS5805M_PAGE_02);
	die_rev = snd_soc_read(codec, TAS5805M_REG_08) & 0x3;
	snd_soc_write(codec, TAS5805M_REG_7E, TAS5805M_REG_00);
	snd_soc_write(codec, TAS5805M_REG_7D, TAS5805M_REG_00);
	snd_soc_write(codec, TAS5805M_REG_00, TAS5805M_PAGE_00);
	snd_soc_write(codec, TAS5805M_REG_7F, TAS5805M_BOOK_00);
	snd_soc_write(codec, TAS5805M_REG_00, TAS5805M_PAGE_00);
	pr_warn("%s(): 5805m die %x detected\n", __func__, die_rev);
	return die_rev;
}
...
static int tas5805m_lpm_mode_locked_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct tas5805m_priv *tas5805m = snd_soc_codec_get_drvdata(codec);
...
static int tas5805m_lpm_mode_locked_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct tas5805m_priv *tas5805m = snd_soc_codec_get_drvdata(codec);
	...
	pr_warn("%s(): LPM mode %s", __func__, enable_lpm_mode ? "Enabled": "Disabled");

	return 0;
}

static const struct snd_kcontrol_new tas5805m_controls [] =
{
	{
		.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
		.name  = "Master TAS5805m Playback Volume",
	...
	SOC_SINGLE_BOOL_EXT("TAS5805m LPM mode", 0,
		tas5805m_lpm_mode_locked_get, tas5805m_lpm_mode_locked_put),
};
	...
	for (i = 0; i < size_seq; i++) {
		...
		if (ret < 0) {
			dev_err(codec->dev, "i2c write failed reg: 0x%x val: 0x%x\n",
				tas5805m_seq[i].reg, tas5805m_seq[i].def);
			return ret;
		}

		// 5ms delay after HiZ state and before programming DSP
		if (apply_hiz_delay &&
			(tas5805m_seq[i].reg == TAS5805M_REG_03 &&
			(tas5805m_seq[i].def & TAS5805M_STATE_MASK))) {
			usleep_range(5000, 5200);
			dev_info(codec->dev, "Delay after HiZ reg: 0x%x val: 0x%x\n",
				tas5805m_seq[i].reg, tas5805m_seq[i].def);
			apply_hiz_delay = false;
		}

		// configurable delay based on the register value
		if (tas5805m_seq[i].reg == CFG_META_DELAY) {
			msleep(tas5805m_seq[i].def);
			dev_info(codec->dev, "Delay for %dms during register programming\n",
				tas5805m_seq[i].def);
		}
...
static int tas5805m_snd_probe(struct snd_soc_codec *codec)
{
	int ret;
	struct tas5805m_priv *tas5805m = snd_soc_codec_get_drvdata(codec);
	const struct reg_sequence *tas5805m_seq1_regs, *tas5805m_seq2_regs;
	int seq1_num_regs, seq2_num_regs;
	tas5805m_seq1_regs = tas5805m_seq2_regs = NULL;
	seq1_num_regs = seq2_num_regs = 0;
	mtk_I2S0dl1_clock_on();

	msleep(100);

	tas5805m->die_rev = tas5805m_get_die_revision(codec);

	switch (tas5805m->speaker_config) {
		case PLATFORM_abc123:
			tas5805m_seq1_regs = &tas5805m_init_sequence1[0];
			seq1_num_regs = ARRAY_SIZE(tas5805m_init_sequence1);
			tas5805m_seq2_regs = &tas5805m_init_sequence2[0];
			seq2_num_regs = ARRAY_SIZE(tas5805m_init_sequence2);
			break;
		case PLATFORM_abc123:
			tas5805m_seq1_regs = &tas5805m_abc123_init_sequence1[0];
			seq1_num_regs = ARRAY_SIZE(tas5805m_abc123_init_sequence1);
			tas5805m_seq2_regs = &tas5805m_abc123_init_sequence2[0];
			seq2_num_regs = ARRAY_SIZE(tas5805m_abc123_init_sequence2);
			break;
	}

	pr_warn("%s(): LPM mode %s", __func__, tas5805m->enable_lpm_mode ? "Enabled": "Disabled");

	ret = tas5805m_apply_seq(codec, tas5805m_seq1_regs, seq1_num_regs);

	if (ret != 0)
	{
		dev_err(tas5805m->dev, "Failed to initialize TAS5805M: %d\n",ret);
		return ret;
	}

	usleep_range(10000, 10200);

	ret = tas5805m_apply_seq(codec, tas5805m_seq2_regs, seq2_num_regs);
	...
	ret = snd_soc_add_codec_controls(codec, tas5805m_controls, ARRAY_SIZE(tas5805m_controls));
...
static int tas5805m_mute(struct snd_soc_dai *dai, int mute)
{
	...
	pr_warn("%s(): 5805m triggered %s mode\n", __func__, mute? "mute": "unmute");
	...
	else
	{
		//unmute
		reg03_value = 0x03;
		reg35_value = tas5805m->swap_lr ? 0x22 : 0x11;
	}
...
static void tas5805m_configure_deepsleep_mode(struct snd_soc_codec *codec, int sleep)
{
	struct tas5805m_priv *tas5805m = snd_soc_codec_get_drvdata(codec);
	int sleep_mode;
	...
	pr_warn("%s(): 5805m triggered %s mode\n", __func__, sleep? (sleep_mode ? "sleep": "deep sleep"): "play");

	if (!sleep)
	{
		snd_soc_write(codec, TAS5805M_REG_02, TAS5805M_DEVICE_BD_MOD_MODE);
		snd_soc_write(codec, TAS5805M_REG_03, TAS5805M_DEVICE_HIZ_MODE);
		usleep_range(2000, 2200);
		snd_soc_write(codec, TAS5805M_REG_02, TAS5805M_DEVICE_HYBRID_MOD_MODE);
		usleep_range(15000, 15200);
		snd_soc_write(codec, TAS5805M_REG_03, TAS5805M_DEVICE_PLAY_MODE);
	}
	else
	{
		snd_soc_write(codec, TAS5805M_REG_03, TAS5805M_DEVICE_HIZ_MODE);
		snd_soc_write(codec, TAS5805M_REG_03, sleep_mode);
	}
}

static int get_lpm_mode_status_locked(struct snd_soc_codec *codec)
{
				 ...
				 struct snd_soc_dai *dai)
{
	int ret = 0;
	struct snd_soc_codec *codec = dai->codec;
	if(get_lpm_mode_status_locked(codec))
		tas5805m_configure_deepsleep_mode(codec, false);
	return ret;
}
				 ...
				 struct snd_soc_dai *dai)
{
	...
	struct snd_soc_codec *codec = dai->codec;
	if(get_lpm_mode_status_locked(codec))
		tas5805m_configure_deepsleep_mode(codec, true);
	return ret;
}
...
static void tas5805m_parse_node(struct tas5805m_priv *tas5805m, struct device *dev)
{
	int ret = 0;
	int enable_lpm_mode = 0;

	ret = of_property_read_u32(dev->of_node, "speaker-config", &tas5805m->speaker_config);
	if(ret){
		dev_err(dev, "%s: Failed to parse dts speaker_config\n",__func__);
	}
	tas5805m->swap_lr = of_property_read_bool(dev->of_node, "swap-channels");

	/* Get tweeter-amp-lpm-mode info */
	if (of_property_read_u32(dev->of_node, "tweeter-amp-lpm-mode", &enable_lpm_mode)) {
			pr_warn("%s: tweeter-amp-lpm-mode is undefined - using default 0\n", __func__);
	} else {
			pr_warn("%s: tweeter-amp-lpm-mode is %d\n", __func__, enable_lpm_mode);
	}
	tas5805m->enable_lpm_mode = enable_lpm_mode;

	/*Get tweeter-amp-reset info*/
	tas5805m->reset_gpio = of_get_named_gpio(dev->of_node, "reset-gpio", 0);
	if (!gpio_is_valid(tas5805m->reset_gpio)) {
		pr_warn("%s: tweeter-amp-reset is undefined\n", __func__);
	}
}
...
static int tas5805m_probe(struct device *dev, struct regmap *regmap)
{
	...
	tas5805m_parse_node(tas5805m,dev);

	/* control power down pin */
	if (gpio_is_valid(tas5805m->reset_gpio)) {
		ret = devm_gpio_request_one(
			dev, tas5805m->reset_gpio,
				GPIOF_OUT_INIT_LOW, "TAS5805m PDN");
		if (ret < 0) {
			dev_err(dev,
				"Failed to request TAS5805m PDN %d: %d\n",
				tas5805m->reset_gpio, ret);
			return ret;
		}

		gpio_set_value_cansleep(tas5805m->reset_gpio, 0);
		msleep(100);
		gpio_set_value_cansleep(tas5805m->reset_gpio, 1);
		msleep(50);
	}
	...
	tas5805m->use_sleep_mode_for_lpm = TAS5805M_DEVICE_DEEPSLEEP_MODE; // By default use deep sleep for lpm
	tas5805m->die_rev = TAS5805M_DIE_B0;  // By default init with die B0 - gets overridden after die revision read
	tas5805m->dev = dev;
...
static int tas5805m_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	/* enable vcn33 power rail*/
	reg = devm_regulator_get(&i2c->dev, "vcn33_wifi");
	if (IS_ERR(reg)) {
		pr_err("%s vcn33_wifi not found\n", __func__);
		return -EINVAL;
	}
	reg_status_before = regulator_is_enabled(reg);
	rc = regulator_enable(reg);
	if (rc != 0) {
		pr_err("Fails to enable vcn33_wifi, ret = 0x%x", rc);
		return -EINVAL;
	}
	reg_status_after = regulator_is_enabled(reg);
	volt = regulator_get_voltage(reg);
	pr_err("%s: vcn33_wifi enable = %d:%d voltage = %d\n", __func__,
		reg_status_before, reg_status_after, volt);

	/* VSIM2 */
	reg = devm_regulator_get(&i2c->dev, "vsim2");
	if (IS_ERR(reg)) {
		pr_err("%s vsim2 not found\n", __func__);
		return -EINVAL;
	}
	reg_status_before = regulator_is_enabled(reg);
	rc = regulator_enable(reg);
	if (rc != 0) {
		pr_err("Fails to enable vsim2, ret = 0x%x", rc);
		return -EINVAL;
	}
	reg_status_after = regulator_is_enabled(reg);
	volt = regulator_get_voltage(reg);
	pr_err("%s: vsim2 enable = %d:%d voltage = %d\n", __func__,
		reg_status_before, reg_status_after, volt);
...
{
	int reset_gpio;
	reset_gpio = of_get_named_gpio(i2c->dev.of_node, "reset-gpio", 0);
