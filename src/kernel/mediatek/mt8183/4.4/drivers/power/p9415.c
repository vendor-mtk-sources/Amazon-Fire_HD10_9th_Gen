...
static u16 p9415_get_vrect_adc(struct p9415_dev *chip)
{
	u8 buf[2] = {0};
	u32 vrect = 0;
...
static void p9415_get_tx_capability(struct p9415_dev *chip)
{
	...
	if (header == 0x3F &&  data_list[0] == 0x63) {
		...
		if (tx_max_iout < 1500000)
			cm->data.wpc_15w_charger_input_current = tx_max_iout;
		else
			cm->data.wpc_15w_charger_input_current = 1500000;
...
static void p9415_report_attach(struct p9415_dev *chip)
{
	struct input_dev *input_dev = chip->input_dev;

	if (IS_ERR_OR_NULL(input_dev)) {
		dev_err(chip->dev, "%s: input device doesn't exist\n",
			__func__);
		return;
	}
...
irqreturn_t p9415_int_handler(int irq, void *ptr)
{
	...
	if (intflag & P9415_INT_VRECTON) {
		dev_info(chip->dev, "%s: Received VRECTON, online\n", __func__);
...
static int p9415_pm_notifier_callback(struct notifier_block *notify,
		unsigned long event, void *data)
{
	struct p9415_dev *chip = container_of(notify,
		struct p9415_dev, pm_notifier);
		...
		const struct i2c_device_id *id)
{
	...
	input_dev->name = "wireless-charger";
	ret = input_register_device(input_dev);
	if (ret)
		goto out_switch;
	chip->input_dev = input_dev;

	chip->pm_notifier.notifier_call = p9415_pm_notifier_callback;
	ret = fb_register_client(&chip->pm_notifier);
	if (ret)
		goto out_input_dev;
	...
	chip->consumer = charger_manager_get_by_name(chip->dev, "wireless-charger");
...
out_pm_notifier:
	fb_unregister_client(&chip->pm_notifier);
out_input_dev:
