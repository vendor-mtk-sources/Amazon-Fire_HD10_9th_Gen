...
{
	if (kstrtoint(str, 10, &silent_ota_inst.is_silent)) {
		pr_err("%s: Failed to convert \"%s\" to integer\n",
			__func__, str);
		return 0;
	}
	pr_info("%s: silent = %d\n", __func__,
			silent_ota_inst.is_silent);
	return 1;
}
...
{
	if (kstrtoint(str, 10, &silent_ota_inst.bl_init_level)) {
		pr_err("%s: Failed to convert \"%s\" to integer\n",
			__func__, str);
		return 0;
	}
	pr_info("%s: bl_init_level = %d\n", __func__,
			silent_ota_inst.bl_init_level);
	return 1;
}
	...
	if (led_cdev) {
		if (on)
			level = led_cdev->brightness ?
					led_cdev->brightness : silent_ota_inst.bl_init_level;
		else
			level = 0;
...
{
	struct keycombo_platform_data *pcombo_cfg =
		silent_ota_keys_cfg.pcombo_cfg;
...
static void __silent_ota_psy_work(struct work_struct *dummy)
{
	...
	if (silent_ota_inst.is_silent && psy) {
		ret = power_supply_get_property(psy, POWER_SUPPLY_PROP_PRESENT, &present);
		if (ret)
			return;

		if (!strncmp(psy->desc->name, "ac", 2) &&
		...
		if (!strncmp(psy->desc->name, "usb", 3) &&
					...
					unsigned long event, void *data)
{
	...
	if (silent_ota_inst.is_silent &&
		event == PSY_EVENT_PROP_CHANGED && psy && psy->desc->name) {
		while (i++ < ARRAY_SIZE(psy_list)) {
...
static int silent_ota_probe(struct platform_device *pdev)
{
	...
	silent_ota_keys_cfg.combo_cfg_size =
		dt_to_keycombo_cfg(pdev, &silent_ota_keys_cfg.pcombo_cfg);
	...
	ret = platform_device_add_data(silent_ota_keys_cfg.pdev_keycombo,
			silent_ota_keys_cfg.pcombo_cfg,
			silent_ota_keys_cfg.combo_cfg_size);
	if (ret)
		goto error;
...
#ifdef CONFIG_OF
static const struct of_device_id silent_ota_dt_match[] = {
